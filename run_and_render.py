...
class fig1_fat(common_process):
    ...
    def blender_process(c, t=32):
        ...
        for key in ['sec', 'remesh']:
            v1,f1 = igl.read_triangle_mesh(f'{c.outpath()}/{t}.{key}.obj')
            blend_surf_file(blendfile=f'{c.base}/render.blend', obj=(use_scale(v1), f1), png=f'{c.base}/{key}.png', plot_edge=True) 
...
if __name__ == '__main__':
    ...
    # render_input(fig1_fat)
    # v,f = igl.read_triangle_mesh(fig1_fat.input)Ore
    # igl.write_triangle_mesh('temp.ply', scale(v), f)
    # for prog in [fig4_lucy_rem]:
    # prog.run([32])
    # prog.log_info()
    # prog.blender_process(32)
